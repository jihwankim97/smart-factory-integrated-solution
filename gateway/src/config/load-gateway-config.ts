...
export const loadGatewayConfig = (baseDir: string): GatewayServiceConfig => {
  const plcReconnectBaseMs = parsePositiveInt(
    process.env.PLC_RECONNECT_BASE_MS,
    1000,
  );
  const plcReconnectMaxMs = parsePositiveInt(
    process.env.PLC_RECONNECT_MAX_MS,
    30000,
  );
  ...
  const devicesConfigRaw = JSON.parse(
    fs.readFileSync(devicesConfigPath, "utf-8"),
  );
  const devicesConfigParsed = devicesSchema.safeParse(devicesConfigRaw);

  if (!devicesConfigParsed.success) {
    throw new Error(
      `Invalid devices config at ${devicesConfigPath}: ${devicesConfigParsed.error.message}`,
    );
  }
  ...
  return {
    ...
    workerPath: path.join(baseDir, "..", "dist", "worker", "data-worker.js"),
    ...
    mqtt: {
      ...
      reconnectBaseMs: parsePositiveInt(
        process.env.MQTT_RECONNECT_BASE_MS,
        1000,
      ),
      reconnectMaxMs: parsePositiveInt(
        process.env.MQTT_RECONNECT_MAX_MS,
        30000,
      ),
    },
  };
